#!/usr/bin/env python3
"""Build the checked gene and molecular-biology question bank payload."""
...
SOURCE = Path("/Users/ray/Downloads/生化_14_小基因_学成题_讲义原话修订版.docx")
OUTPUT = Path("src/data/biochemistry-lecture14-data.json")
LECTURE_NUMBER = 14
TITLE = "生化 小基因"
TOPIC = "小基因"
...
def parse_workbook():
    ...
    for paragraph in document.paragraphs:
        ...
        if mode == "answers":
            ...
            if answer_group_match:
                ...
                answer_text = answer_group_match.group(2)
                for number, answer in ANSWER_RE.findall(answer_text):
    ...
    for group, options in zip(groups, option_banks):
        ...
        if set(group["answers"]) != expected_numbers:
            raise ValueError(f"Group {group['source_index']}: expected {sorted(expected_numbers)}, got {sorted(group['answers'])}")
...
def evidence(page):
    return {
        ...
        "lectureTitle": TITLE,
        "page": page,
        "image": f"biochemistry/lecture-pages/lecture-14-page-{page:02d}.webp",
        "title": f"第 14 讲《{TITLE}》· 第 {page} 页",
        "description": "已按小基因讲义、思维导图与生化合集逐项核对；点击可查看讲义原页。",
        "method": "按 2027 考研生化第 14 讲《小基因》及配套思维导图逐项复核。",
    }
...
def make_group(source_group, display_index, evidence_page):
    ...
    return {
        ...
        "reviewState": "已按小基因讲义、思维导图与生化合集核对",
        "reviewIssues": [],
        "reviewNotes": [],
        "topic": TOPIC,
        "lectureIds": ["lecture-14"],
...
def main():
    ...
    evidence_pages = {
        ...
        **{index: 2 for index in range(7, 13)},
        **{index: 3 for index in range(13, 17)},
        **{index: 4 for index in range(17, 19)},
    }
    ...
    groups.extend(make_fill_groups(
        ...
        ranges=[(1, 6, 1), (7, 10, 2), (11, 12, 3)],
        evidence_for_page=evidence,
        review_state="已按小基因 DOCX 原文逐空核对",
    ))
    payload = {
        "meta": {
            "title": "生物化学第 14 讲题库",
            "sourceLabel": "生化第 14 讲学成题（小基因）",
            "sourcePages": 4,
            "lectureCount": 1,
            "groupCount": len(groups),
            "stemCount": sum(len(group["stems"]) for group in groups),
            "correctionGroupCount": 0,
            "generatedBy": "scripts/build_biochemistry_lecture14.py",
            "siteIntegrated": True,
            "lectureLinked": True,
            "answerNote": "完整收录第 14 讲《小基因》选择题与填空题；选择题选项已打散，填空题按 DOCX 原编号与原答案导入。",
        },
        "topics": ["全部", TOPIC, "综合"],
        "pages": [{"page": group["page"], "image": "", "topic": TOPIC, "searchText": group["title"]} for group in groups],
        "groups": groups,
        "lectures": [{"id": "lecture-14", "number": LECTURE_NUMBER, "title": TITLE, "pageCount": 4}],
    }
