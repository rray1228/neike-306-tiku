...
def main():
    groups = [
        ...
        group(4, "脂肪动员的酶与调节", [
            "激素敏感性甘油三酯酯酶（HSL）", "脂肪→二酰甘油+脂肪酸", "抗脂解激素", "ATGL",
            "二酰甘油→甘油一酯+脂肪酸", "胰岛素", "儿茶酚胺", "甘油一酯脂肪酶",
            "前列腺素", "胰高血糖素", "脂解激素", "甘油一酯→甘油+脂肪酸",
        ], [
            ("ATGL 催化", ["脂肪→二酰甘油+脂肪酸"]),
            ("HSL 催化", ["二酰甘油→甘油一酯+脂肪酸"]),
            ("HSL 的激活剂", ["儿茶酚胺", "胰高血糖素", "脂解激素"]),
            ("HSL 的抑制剂", ["抗脂解激素", "胰岛素", "前列腺素"]),
            ("甘油一酯脂肪酶催化", ["甘油一酯→甘油+脂肪酸"]),
        ], 2),
        group(5, "甘油的利用", [
            "甘油激酶", "消耗 2 个 ATP", "肝活性最高", "合成脂肪（除脂肪和骨骼肌细胞）",
            "1 分子甘油完全分解", "生成 18.5/16.5 个 ATP", "脂肪和骨骼肌细胞活性很低甚至缺乏", "2 分子甘油糖异生",
        ], [
            ("甘油的利用", ["合成脂肪（除脂肪和骨骼肌细胞）", "1 分子甘油完全分解", "2 分子甘油糖异生"]),
            ("2 分子甘油糖异生", ["消耗 2 个 ATP"]),
            ("1 分子甘油完全分解", ["生成 18.5/16.5 个 ATP"]),
            ("甘油激酶的特点", ["肝活性最高", "脂肪和骨骼肌细胞活性很低甚至缺乏"]),
        ], 2),
