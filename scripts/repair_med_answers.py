...
def corrected_group_blocks() -> dict[str, tuple[list[str], list[dict]]]:
    ...
    valve_options = [
        ...
        option("a", "相对性二尖瓣关闭不全"), option("b", "相对性主动脉瓣关闭不全"), option("c", "S1多正常，也可↓或↑"), option("d", "S1↑"), option("e", "S1↓"), option("f", "S2逆分裂"), option("g", "S2通常分裂"), option("h", "A2↓"), option("i", "S3奔马律"), option("j", "开瓣音"), option("k", "S4奔马律"), option("l", "心尖收缩中晚期喀喇音"), option("m", "易左心衰、房颤、脑栓塞、感染性心内膜炎"), option("n", "最易左室肥厚、心绞痛、晕厥"), option("o", "最易左心衰、房颤、脑栓塞"), option("p", "最易感染性心内膜炎"), option("q", "首选和金标准：超声心动图"), option("r", "慢性者球形心"), option("s", "慢性者靴形心"), option("t", "梨形心"), option("u", "心影一般不大或靴形心，升主动脉扩张"), option("v", "置换术"), option("w", "有开瓣音：经皮球囊二尖瓣成形术"), option("x", "成人置换术，老龄重度不耐受TAVR"), option("y", "儿童青少年钙化性少：分离术"), option("z", "合并二闭等：置换术"), option("①", "修复术"), option("②", "无开瓣音：直视分离术"),
    ]
    p85_valves = group(
